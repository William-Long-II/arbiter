...
- `<!-- reviewme:verdict=approve -->` — no blocking issues at any of the above tiers; safe to merge.
- `<!-- reviewme:verdict=comment -->` — non-blocking observations only.
- `<!-- reviewme:verdict=request-changes -->` — at least one blocking issue (bug, security, contract break, or missing test for non-trivial logic).
