...
- `<!-- arbiter:verdict=request-changes -->` — at least one issue must be addressed before merge. You must be able to name the change the author has to make. If you are instead asking them to confirm or check something, that is a question, not a blocker — never block a pull request on a question.
