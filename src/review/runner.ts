...
export function formatSubscriptionPreflightError(detail: string): string {
  return [
    ...
    '  │  which needs credentials: either CLAUDE_CODE_OAUTH_TOKEN in .env,',
    '  │  or the host\'s Claude Code login bind-mounted at /root/.claude.',
    '  │',
    '  │  Headless/server (recommended): run `claude setup-token` on any',
    '  │  machine where you\'re logged in, paste the sk-ant-oat... token',
    '  │  into .env as CLAUDE_CODE_OAUTH_TOKEN, and recreate the app.',
    '  │',
    '  │  Or fix the bind-mount on the HOST:',
    '  │',
