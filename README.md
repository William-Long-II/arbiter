...
- **Webhook body-size cap + per-IP ingress rate limit.** 5 MB ceiling on request body (real GitHub payloads top out well below this); `Content-Length` check rejects oversize payloads before reading. Token-bucket limiter defaulting to 120 burst + 2/sec sustained per client IP; a retry storm or drive-by scan gets 429 + `Retry-After` instead of exhausting process memory.
- **GitHub API rate-limit visibility.** Every REST response's `X-RateLimit-*` headers land on the Dashboard as `remaining / limit · reset in N minutes`, so you can spot a runaway discovery pass before the PAT's 5000/hr quota runs dry.
...
- **Deep healthcheck.** `/healthz` returns a JSON body reporting sqlite reachability, review-loop liveness (last tick within 3× poll interval, floor 60s), circuit-breaker state, config readiness, and SQLite integrity (from the boot-time `PRAGMA integrity_check`). HTTP 503 only when sqlite is broken or the integrity check failed; other degraded states return 200 so a container orchestrator doesn't kill the pod on a temporary breaker open. The Docker image's `HEALTHCHECK` directive uses this endpoint; `/api/version` reports the running commit.
- **SQLite integrity check at boot.** Every time the container starts on an existing DB, `PRAGMA integrity_check` runs. A non-ok result is logged loudly, surfaced as an error event + a red banner on the Dashboard, and flips `/healthz` to `down`. The process keeps running so an operator can still hit `/api/backup` and `scripts/restore.sh`.
- **One-click backup.** `/api/backup` (admin-only) streams a consistent SQLite snapshot built via `VACUUM INTO` — same mechanism `scripts/backup.sh` uses, but reachable from the Dashboard's Storage card without needing SSH. Schedule recurring backups via cron calling `scripts/backup.sh`.
